...
class DataPointAnalyser {
    ...
    static analyze({ transformedData, emaWindows, position, item, confirm, }) {
        ...
        const signalLine = closingMACD.reduce((sum, price) => sum + price, 0) / signalWindow;
        return {
            timestamp: this.formatDate(timestamp),
            open,
            close,
    ...
    static calculateEMA(windowSize, previousEMA, close) {
        if (!previousEMA)
            return 0;
...
exports.default = DataPointAnalyser;
